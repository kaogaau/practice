def insert_comments(mysql_client,feeds_set)
	set = feeds_set.clone
	...
	success_count = 0
	set.each do |doc|
		id = nil,date= nil ,message = nil,feed_id = nil,user_id = nil
		if doc['doc']['comments']['data'].size > 0
			doc['doc']['comments']['data'].each do |ele|
				begin
					...
					feed_id = doc['_id'].split('_')[1]
					user_id = ele['from']['id']
					insert_str = "INSERT INTO comments (date,message,feed_id,user_id) VALUES (\'#{date}\',\'#{message}\',#{feed_id.to_i},#{user_id.to_i})"
