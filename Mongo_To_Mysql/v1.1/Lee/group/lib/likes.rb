def insert_likes(mysql_client,feeds_set)
	set = feeds_set.clone
	...
	success_count = 0
	set.each do |doc|
		id = nil,feed_id = nil,user_id = nil
		if doc['doc']['likes']['data'].size > 0
			doc['doc']['likes']['data'].each do |ele|
				begin
					feed_id = doc['doc']['id'].split('_')[1]
					user_id = ele['id']
					insert_str = "INSERT INTO likes (feed_id,user_id) VALUES (#{feed_id.to_i},#{user_id.to_i})"
